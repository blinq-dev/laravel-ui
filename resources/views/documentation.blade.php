    ...
    <div class="wire:text-lg mt-8 grow-0 shrink-0">
        <div class="flex flex-col gap-12" x-data="{
            ...
            <div class="flex flex-row flex-wrap gap-2">
                @foreach ($props as $prop)
                    <div class="flex flex-row items-center gap-4 w-full">
                        ...
                        @else
                            <label class="text-left text-sm mr-4 w-32 shrink-0">{{ $name }}</label>
                            <x-input x-model="props.{{ $name }}" size="sm" bg="" ring="gray-50 grow w-full" />
                        @endif
                    </div>
                @endforeach
            </div>
            ...
            <div class="flex gap-10 mb-10 flex-col">
                ...
                {{-- Preview code --}}
                <x-textarea :copyable="true" class="font-mono border-gray text-black focus:outline-none rounded-xl text-sm w-full" x-model="code" rows="3"></x-textarea>
            </div>
        </div>
    </div>
